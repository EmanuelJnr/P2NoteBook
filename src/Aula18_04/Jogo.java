...
public class Jogo {
	...
	public static boolean testeSeAcabou(Domino[] pecas) {
		int cont=0;
		for (Domino d : pecas) {
			if(d==null)
				cont++;
		}
		if(cont==pecas.length)
			return true;
		return false;
	}
	
	public boolean conectarPontaEsq(Domino d) {
		if(pontaAtivaDireita==-1 || pontaAtivaEsquerda==-1) {
			pontaAtivaEsquerda = d.getLadoA();
			pontaAtivaDireita = d.getLadoB();
			return true;
		}
		else if(pontaAtivaEsquerda==d.getLadoA()){
	...
	public boolean conectarPontaDir(Domino d) {
		if(pontaAtivaDireita==-1 || pontaAtivaEsquerda==-1) {
			pontaAtivaEsquerda = d.getLadoA();
			pontaAtivaDireita = d.getLadoB();
			return true;
		}
		else if(pontaAtivaDireita==d.getLadoA()){
			pontaAtivaDireita = d.getLadoB();
			return true;
		}
